...
struct document get_document(char* text) {
    ...
    struct document ret_doc = doc_init();
    int pc = 0;int sc=0;int wc=0;int cc=0;
    
    for (int i=0 ; text[i] != '\0'; i++) {
        if(text[i] == ' ')
        {
            ret_doc.data[pc].data[sc].word_count++;
            wc++;
            cc=0;
            continue;
        } 
        ...
        {
            ret_doc.data[pc].data[sc].word_count++;
            ret_doc.data[pc].sentence_count++;
            sc++;
            wc=0;
            cc=0;
            continue;
        ...
        ret_doc.data[pc].data[sc].data[wc].data[cc] = text[i];
        cc++;
    }
